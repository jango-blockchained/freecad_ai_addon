...
def test_pattern_recognizer():
    ...
    if patterns:
        ...
        print(f"  - Suggested dimensions: {len(best_pattern.suggested_dimensions)} parameters")
    ...
    if flange_patterns and flange_patterns[0].pattern_type == PatternType.FLANGE_CONNECTION:
...
def test_optimization_engine():
    ...
    strength_result = optimizer.optimize_parameters(
        OptimizationGoal.MAXIMIZE_STRENGTH,
        parameters
    )
...
def test_error_recovery():
    ...
    strategies = recovery.handle_error("boolean_operation_failed", error_details, context)
    ...
    constraint_error = {
        "type": "sketch_constraint_conflict",
        "conflicting_value": 25.0
    }
...
def test_full_decision_engine():
    ...
    if result['patterns_recognized']:
        ...
        print(f"  - Best pattern: {best_pattern.pattern_type.value} ({best_pattern.confidence:.1%})")
    
    if result['optimization_results']:
        opt = result['optimization_results']
        print(f"  - Optimization: {opt.objective_value:.2f} in {opt.iterations} iterations")
