...
class TestProviderManagementSystem:
    ...
    async def test_provider_status_monitoring(self):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            self.credential_manager.store_credential("openai", "api_key", "sk-test-key-12345678901234567890")
    ...
    async def test_connection_manager_retry_logic(self):
        ...
        async def mock_check_connection(provider):
            ...
            if call_count == 1:
                # First call fails
                from freecad_ai_addon.core.provider_status import ProviderHealth, ProviderStatus
        ...
        with patch.object(connection_manager.provider_monitor, 'check_provider_connection', side_effect=mock_check_connection):
            # Store test credentials
            self.credential_manager.store_credential("openai", "api_key", "sk-test-key-12345678901234567890")
    ...
    async def test_connection_manager_fallback(self):
        ...
        with patch.object(connection_manager.provider_monitor, 'check_provider_connection', side_effect=mock_check_connection):
            ...
            self.credential_manager.store_credential("openai", "api_key", "sk-test-key-12345678901234567890")
            self.credential_manager.store_credential("anthropic", "api_key", "sk-ant-test-key-12345678901234567890")
    ...
    def test_integration_provider_lifecycle(self):
        ...
        assert self.credential_manager.store_credential(provider, "api_key", "sk-test-key-12345678901234567890")
        assert self.credential_manager.store_credential(provider, "org_id", "org-test123")
