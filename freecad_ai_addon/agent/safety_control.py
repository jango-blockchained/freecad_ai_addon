...
class AgentSafetyController:
    ...
    def validate_operation(self, task: AgentTask,
            ...
            try:
                if not constraint.check_function(task, context):
                    ...
                    if constraint.risk_level.value in ["high_risk", "destructive"]:
                        result.errors.append(f"Safety constraint failed: {constraint.description}")
                        ...
                        if self._compare_risk_level(constraint.risk_level, result.risk_level):
                            result.risk_level = constraint.risk_level
                    
                    # Add auto-fix if available
                    if constraint.auto_fix_available:
                        result.auto_fixes_available.append(constraint.name)
                        result.suggestions.append(f"Auto-fix available for: {constraint.name}")
                
            except Exception as e:
                self.logger.error(f"Error checking constraint {constraint.name}: {str(e)}")
                result.warnings.append(f"Could not verify constraint: {constraint.name}")
    ...
    def require_user_confirmation(self, task: AgentTask, safety_result: SafetyCheckResult, 
        ...
        if not Gui or QDialog is None:
            ...
            if (safety_result.risk_level in [OperationRisk.HIGH_RISK, OperationRisk.DESTRUCTIVE] or
        ...
        needs_confirmation = (
            safety_result.risk_level in [OperationRisk.HIGH_RISK, OperationRisk.DESTRUCTIVE] or
            self.safety_level == SafetyLevel.CRITICAL or
            bool(safety_result.errors)
        )
        ...
        self.logger.info(f"User {'confirmed' if confirmed else 'cancelled'} operation: {task.description}")
        
        return confirmed
    
    def create_operation_preview(self, task: AgentTask, context: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        ...
        if task.task_type.value in ['box', 'cylinder', 'sphere']:
            preview['preview_objects'] = [f"Preview_{task.parameters.get('name', 'Object')}"]
    ...
    def setup_rollback_point(self, operation_id: str, context: Dict[str, Any] = None) -> str:
        """
    ...
    def show_manual_override_dialog(self, agent_status: Dict[str, Any]) -> Optional[str]:
        """
    ...
    def _initialize_safety_constraints(self) -> List[SafetyConstraint]:
        ...
        constraints.append(SafetyConstraint(
            ...
            check_function=lambda task, ctx: App is not None and App.ActiveDocument is not None,
        ...
        def check_object_exists(task: AgentTask, context: Dict[str, Any]) -> bool:
            if task.task_type.value in ['add_fillet', 'add_chamfer', 'boolean_union', 'boolean_difference']:
                obj_name = task.parameters.get('obj_name') or task.parameters.get('objects', [])
                if isinstance(obj_name, str):
                    return App.ActiveDocument.getObject(obj_name) is not None
                elif isinstance(obj_name, list):
                    return all(App.ActiveDocument.getObject(name) is not None for name in obj_name)
            return True
        ...
        def check_destructive_operation(task: AgentTask, context: Dict[str, Any]) -> bool:
    ...
    def _check_resource_limits(self, task: AgentTask, context: Dict[str, Any], 
                             result: SafetyCheckResult):
    ...
    def _get_affected_objects(self, task: AgentTask, context: Dict[str, Any]) -> List[str]:
    ...
    def _calculate_geometry_preview(self, task: AgentTask) -> Dict[str, Any]:
        ...
        if task.task_type.value == 'box':
            params = task.parameters
            volume = params.get('length', 0) * params.get('width', 0) * params.get('height', 0)
