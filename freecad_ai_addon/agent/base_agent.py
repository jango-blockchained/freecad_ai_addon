...
class BaseAgent(ABC):
    ...
    def execute_with_safety(self, task: AgentTask) -> TaskResult:
        ...
        try:
            ...
            safety_result = self._safety_controller.validate_operation(
                task, task.context
            )
            ...
            if not self._safety_controller.require_user_confirmation(
                task, safety_result, task.context
            ):
                return TaskResult(
            ...
            if not self._safety_controller.check_resource_limits(task):
                return TaskResult(
                    status=TaskStatus.FAILED, error_message="Resource limits exceeded"
                )

            # Create rollback point for critical operations
            rollback_id = None
            if safety_result.risk_level.value in ["high_risk", "destructive"]:
                rollback_id = self._safety_controller.setup_rollback_point(
