...
class SketchAgent(BaseAgent):
    ...
    def _add_rectangle(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Bottom line
        line_ids.append(sketch.addGeometry(
            Part.LineSegment(
                App.Vector(x1, y1, 0),
                App.Vector(x2, y1, 0)
            ), construction
        ...
        sketch.addConstraint(Sketcher.Constraint(
            'Coincident', line_ids[0], 2, line_ids[1], 1
        ))
        sketch.addConstraint(Sketcher.Constraint(
            'Coincident', line_ids[1], 2, line_ids[2], 1
        ))
        sketch.addConstraint(Sketcher.Constraint(
            'Coincident', line_ids[2], 2, line_ids[3], 1
        ))
        sketch.addConstraint(Sketcher.Constraint(
            'Coincident', line_ids[3], 2, line_ids[0], 1
        ))
    ...
    def _add_point(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        point_id = sketch.addGeometry(
            Part.Point(App.Vector(position[0], position[1], 0)),
            construction
        )
            ...
            for i, geom in enumerate(sketch.Geometry):
                if hasattr(geom, 'StartPoint'):
                    ...
                    sketch.addConstraint(
                        Sketcher.Constraint('Coincident', i, 1, -1, 1)
                    )
                    break
