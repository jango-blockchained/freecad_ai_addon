    ...
    async def _execute_with_monitoring(self):
        ...
        def approval_callback(operation: str, details: dict) -> bool:
            ...
            auto_approve = (
                ...
                and not (
                    is_modification
                    and self.settings.require_confirmation_for_modifications
                )
            )
            if auto_approve:
                return True
...
class AgentControlPanel(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.confirm_modifications_cb = QCheckBox("Confirm model modifications")
    ...
    def _update_settings(self):
        ...
        self.settings.auto_approve_safe_operations = self.auto_approve_cb.isChecked()
        self.settings.require_confirmation_for_modifications = (
            self.confirm_modifications_cb.isChecked()
        )
        self.settings.enable_safety_validation = self.enable_safety_cb.isChecked()
        self.settings.max_operations_per_task = self.max_ops_combo.currentData()
    ...
    def set_settings(self, settings: AgentControlSettings):
        ...
        # Update combo boxes
        self.max_ops_combo.setCurrentText(str(settings.max_operations_per_task))
...
class AgentConversationIntegration(QObject):
    ...
    def _setup_integration(self):
        ...
        self.conversation_widget.message_sent.connect(self._handle_user_message)
    ...
    async def _initialize_framework(self):
        ...
        try:
            ...
            self._add_system_message("Agent framework initialized successfully")
            agents_list = ", ".join(self.agent_framework.get_available_agents())
            self._add_system_message(f"Available agents: {agents_list}")
        except Exception as e:
    ...
    def _handle_user_message(self, text: str, attachments: List[str]):
        ...
        if settings.mode != AgentMode.DISABLED and contains_agent_request:
            ...
            self._add_system_message("Message passed to AI provider (agents disabled)")

    def _execute_agent_task(self, task_text: str, settings: AgentControlSettings):
        ...
        self.current_thread.operation_pending.connect(self._on_operation_pending)
    ...
    def _on_operation_pending(self, task_id: str, operation: str, details: dict):
    ...
    def _add_approval_message(self, message: str, task_id: str, operation: str):
    ...
    def _on_settings_changed(self, settings: AgentControlSettings):
        ...
        self._add_system_message(f"Agent mode changed to: {settings.mode.value}")
    ...
    def _add_assistant_message(self, text: str):
        ...
        self.conversation_widget.add_assistant_message(text, provider="AI Agent")
